...
export default class UploadTorrent extends Component {
  ...
  titleClick = (e, { title }) => {
    this.setState({ active: title }, this.refreshForm);
  }

  next = () => {
    if(this.state.active === 'Torrent Info') {
      this.setState({active: 'Trackers'}, this.refreshForm);
    } else if(this.state.active === 'Trackers') {
      this.setState({active: 'Additional Info'}, this.refreshForm);
    }
  }

  previous = () => {
    if(this.state.active === 'Trackers') {
      this.setState({active: 'Torrent Info'}, this.refreshForm);
    } else if(this.state.active === 'Additional Info') {
      this.setState({active: 'Trackers'}, this.refreshForm);
    }
  }
  ...
  render() {
    ...
    const categories = [
      { key: '0', text: 'movies', value: 'movies' },
      { key: '1', text: 'tv/shows', value: 'tv/shows' },
      { key: '2', text: 'games', value: 'games' },
      { key: '3', text: 'music', value: 'music' },
      { key: '4', text: 'applications', value: 'applications' },
      { key: '5', text: 'documents', value: 'documents' },
      { key: '6', text: 'movies', value: 'movies' },
      { key: '7', text: 'xxx', value: 'xxx' },
    ];
        ...
        <Card.Content >
          ...
          <Form>
            ...
            <Button.Group attached='bottom' className='stepperButtons'>
              <Button onClick={this.previous} style={{display: this.state.previous }}><Icon color='black' name='arrow circle left' />Previous</Button>
              <Button onClick={this.next} style={{display: this.state.next }}>Next<Icon color='black' name='arrow circle right' /></Button>
              <Button onClick={this.share} style={{display: this.state.stepAdditionalInfo }}>Share<Icon color='black' name='arrow circle right' /></Button>
            </Button.Group>
          </Form>
        </Card.Content>
      </Card>
