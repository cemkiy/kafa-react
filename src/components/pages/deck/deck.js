...
export default class Deck extends Component {
  ...
  joinUsClick = () => {
    this.setState({joinUsFormDisplay: "block", signInFormDisplay: "none", forgotPassFormDisplay: "none", formResultDisplay: "none"});
  }

  signInClick = () => {
    this.setState({signInFormDisplay: "block", joinUsFormDisplay: "none", forgotPassFormDisplay: "none", formResultDisplay: "none"});
  }

  forgotPassClick = () => {
    this.setState({forgotPassFormDisplay: "block", signInFormDisplay: "none", joinUsFormDisplay: "none", formResultDisplay: "none"});
  }

  joinUs = (event) => {
    CreateUser(this.state.joinUsFormData, ['id', 'username', 'email', 'birthday', 'created_at']).then(data => {
      this.setState({formResultDisplay: "block", formResultType: 'green', formResultHeader: 'Register Successfull', formResultDescription: 'Before login, go to your email and click the activation button.'});
    }).catch(err => {
      ErrorAnalysis(err, this.props.history);
      this.setState({formResultDisplay: "block", formResultType: 'red', formResultHeader: 'Register Failed', formResultDescription: err.response.errors[0].message});
    });
  ...
  signIn = (event) => {
    CreateToken(this.state.signInFormData, [
      ...
      this.setState({formResultDisplay: "block", formResultType: 'red', formResultHeader: 'Sign in Failed', formResultDescription: err.response.error});
    });
    event.preventDefault();
  }
  ...
  forgotPass = (event) => {
    ForgotPass(this.state.forgotPassFormData, []).then(data => {
      this.setState({formResultDisplay: "block", formResultType: 'green', formResultHeader: 'Email Sended', formResultDescription: 'Please check your email and change your password.'});
    }).catch(err => {
  ...
  render() {
    return (<div className='center'>
      <Grid columns={2}>
          ...
          <Grid.Column width={13}>
            ...
            <div>
              ...
              <List bulleted="bulleted">
                <List.Item>The ratio should not fall below
                  <strong>0.5</strong>.</List.Item>
                <List.Item>You should have
                  <strong>min 1 GB</strong>
                  download/upload every month.</List.Item>
                <List.Item>You must do it
                  <strong>on site</strong>
                  for the counting of uploads size.</List.Item>
              </List>
            </div>
        ...
        <Grid.Row style={{
            display: this.state.joinUsFormDisplay
          }}>
          <Grid.Column width={16}>
            <Form onSubmit={this.joinUs}>
              <Form.Group widths='equal'>
                <Form.Input fluid="fluid" name="username" label='username' value={this.state.joinUsFormData.username} onChange={this.joinUsFormHandleChange} placeholder='captainjack' required="required"/>
                <Form.Input fluid="fluid" name='email' type='email' label='email' value={this.state.joinUsFormData.email} onChange={this.joinUsFormHandleChange} placeholder='jacksparrow@blackpearl.com' required="required"/>
              </Form.Group>
              <Form.Group widths='equal'>
                <Form.Input fluid="fluid" type='password' name='password' label='password' value={this.state.joinUsFormData.password} onChange={this.joinUsFormHandleChange} placeholder='min 8, have a number' required="required"/>
                <Form.Input fluid="fluid" type='password' label='password again' placeholder='********' required="required"/>
              </Form.Group>
              <Message>
                <div>The reason we do want your birthday, verify your age is 18 or bigger.</div>
                <Form.Input fluid="fluid" name='birthday' label='birthday' value={this.state.joinUsFormData.birthday} onChange={this.joinUsFormHandleChange} placeholder='YYYY-mm-dd exp:1993-03-22' required="required"/>
              </Message>
              <Form.Field required="required">
                <Checkbox onChange={this.termsAgreeChange} label='I agree to the Terms and Conditions'/>
              </Form.Field>
              <Form.Field>
                <Recaptcha sitekey="6LcRq1QUAAAAAGsVoN2J52MW7C9dgkmhC1IY-Dxx" verifyCallback={this.verifyCallback}/>
              </Form.Field>
        ...
        <Grid.Row style={{
            display: this.state.signInFormDisplay
          }}>
          <Grid.Column width={16}>
            <Form onSubmit={this.signIn}>
              <Form.Group widths='equal'>
                <Form.Input fluid="fluid" name='usernameOrEmail' label='username or email' value={this.state.signInFormData.usernameOrEmail} onChange={this.signInFormHandleChange} placeholder='captainjack or jacksparrow@blackpearl.com' required="required"/>
                <Form.Input fluid="fluid" type='password' name='password' label='password' value={this.state.signInFormData.password} onChange={this.signInFormHandleChange} placeholder='********' required="required"/>
              </Form.Group>
              <Form.Group>
        ...
        <Grid.Row style={{
            display: this.state.forgotPassFormDisplay
          }}>
          <Grid.Column width={16}>
            <Form onSubmit={this.forgotPass}>
              <Form.Group widths='equal'>
                <Form.Input fluid="fluid" name='email' type='email' label='email' value={this.state.forgotPassFormData.email} onChange={this.forgotPassFormHandleChange} placeholder='jacksparrow@blackpearl.com' required="required"/>
              </Form.Group>
              <Form.Group>
            ...
            <Item.Group>
              <Item>
                <Item.Content verticalAlign='middle'>
                  ...
                  <Item.Description>
                    kafa.io is a bittorrent client. You can download/upload operations on this site.[coming soon]
                  </Item.Description>
                </Item.Content>
              </Item>
              <Item>
