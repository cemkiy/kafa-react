...
export default class Profile extends Component {
  ...
  componentWillMount() {
      UserById({id: this.props.match.params.userId},
        ['id', 'username', 'email', 'about', {'role': ['type']}, 'birthday',
        'created_at']).then(data => {
  ...
  render() {
    const panes = [
      ...
      <Tab.Pane>
        ...
        <Header as='h5'>Type:</Header> { this.state.user.role ? this.state.user.role.type : '' }
